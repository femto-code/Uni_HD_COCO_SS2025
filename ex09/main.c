...
double threaded_benchmark(RBTNode **root, int stream_size, float insert_ratio, int num_threads) {
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <stream_size> <thread_count> <insert_ratio>\n", argv[0]);
        return 1;
    }

    int stream_size = atoi(argv[1]);
    int thread_count = atoi(argv[2]);
    float insert_ratio = atof(argv[3]);
    ...
    for (int iter = 0; iter < 2; iter++) {
        RBTNode *root = NULL;
        init_rbt(&root, initial_elements);
        if (!isValidRBT(root)) {
            printf("Invalid RBT!\n");
            return 1;
        }
        double duration = threaded_benchmark(&root, stream_size, insert_ratio, thread_count);
        total_duration += duration;
        free_rbt(root);
    }
    double avg_duration = total_duration / 2.0;
    printf("1,%d,%.1f,%d,%.4f,%.2f\n", thread_count, insert_ratio, stream_size, avg_duration, stream_size / avg_duration);
